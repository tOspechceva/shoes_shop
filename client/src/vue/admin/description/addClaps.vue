<template> 
  ...
  <div class="main">
    <div class="container">
      <h1>ДОБАВЛЕНИЕ</h1>
      <label for="description">Застежка:</label>
      <input
        type="text"
        name="description"
        v-model="description"
        placeholder="Введите название застежки"
      />
...
export default {
    ...
    async add() {
        try {
            await AdminService.addClaps({ name: this.description });
            this.error = "Застежка добавлена";
            this.getAll(); // Обновляем список после добавления
        } catch (error) {
            this.error = error.response?.data?.error || "Ошибка при добавлении застежки";
        }
    },
    async getAll() {
        try {
            const response = await AdminService.getClaps();
            this.products = response.data; // Исправлено, убран лишний `if`
        } catch (error) {
            this.error = error.response?.data?.error || "Ошибка при получении застежек";
        }
    },
    async update() {
        try {
            ...
            await AdminService.updateClaps(id, { name });
            this.error = "Застежка обновлена";
            this.getAll(); // Обновляем список после изменения
        } catch (error) {
            this.error = error.response?.data?.error || "Ошибка при обновлении застежки";
        }
    },
    async delete_() {
        try {
            const id = this.delete_v; // Ожидаем ввод ID
            await AdminService.deleteClaps(id);
            this.error = "Застежка удалена";
            this.getAll(); // Обновляем список после удаления
        } catch (error) {
            this.error = error.response?.data?.error || "Ошибка при удалении застежки";
        }
    }
    }
}
