...
export default {
    ...
    async add() {
        try {
            await AdminService.addSize({ name: this.description });
            this.error = "Размер добавлен";
            this.getAll(); // Обновляем список после добавления
        } catch (error) {
            this.error = error.response?.data?.error || "Ошибка при добавлении размера";
        }
    },
    async getAll() {
        try {
            const response = await AdminService.getSize();
            this.products = response.data; // Исправлено, убран лишний `if`
        } catch (error) {
            this.error = error.response?.data?.error || "Ошибка при получении размера";
        }
    },
    async update() {
        try {
            ...
            await AdminService.updateSize(id, { name });
            this.error = "Размер обновлен";
            this.getAll(); // Обновляем список после изменения
        } catch (error) {
            this.error = error.response?.data?.error || "Ошибка при обновлении размера";
        }
    },
    async delete_() {
        try {
            ...
            this.error = "Размер удален";
            this.getAll(); // Обновляем список после удаления
        } catch (error) {
            this.error = error.response?.data?.error || "Ошибка при удалении размера";
        }
    }
    },
  created() {
