<template> 
  ...
  <div class="main">
    <div class="container">
      <h1>ДОБАВЛЕНИЕ</h1>
      <label for="description">Утеплитель:</label>
      <input
        type="text"
        name="description"
        v-model="description"
    ...
    <button class="show-button" @click="getAll">Показать все утеплители</button>
...
export default {
    ...
    async add() {
        try {
            await AdminService.addInsulation({ name: this.description });
            this.error = "Утеплитель добавлен";
            this.getAll(); // Обновляем список после добавления
        } catch (error) {
            this.error = error.response?.data?.error || "Ошибка при добавлении утеплителя";
        }
    },
    async getAll() {
        try {
            const response = await AdminService.getInsulation();
            this.products = response.data; // Исправлено, убран лишний `if`
        } catch (error) {
            this.error = error.response?.data?.error || "Ошибка при получении утеплителя";
        }
    },
    async update() {
        try {
            ...
            await AdminService.updateInsulation(id, { name });
            this.error = "Утеплитель обновлен";
            this.getAll(); // Обновляем список после изменения
        } catch (error) {
            this.error = error.response?.data?.error || "Ошибка при обновлении утеплителя";
        }
    },
    async delete_() {
        try {
            const id = this.delete_v; // Ожидаем ввод ID
            await AdminService.deleteInsulation(id);
            this.error = "Утеплитель удален";
            this.getAll(); // Обновляем список после удаления
        } catch (error) {
            this.error = error.response?.data?.error || "Ошибка при удалении утеплителя";
        }
    }
    }
}
