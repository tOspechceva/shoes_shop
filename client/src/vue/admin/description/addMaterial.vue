<template> 
  ...
  <div class="main">
    ...
    <button class="show-button" @click="getAll">Показать все материалы</button>
...
export default {
    ...
    async add() {
        try {
            await AdminService.addMaterial({ name: this.description });
            this.error = "Материал добавлен";
            this.getAll(); // Обновляем список после добавления
        } catch (error) {
            this.error = error.response?.data?.error || "Ошибка при добавлении материала";
        }
    },
    async getAll() {
        try {
            const response = await AdminService.getMaterial();
            this.products = response.data; // Исправлено, убран лишний `if`
        } catch (error) {
            this.error = error.response?.data?.error || "Ошибка при получении материала";
        }
    },
    async update() {
        try {
            ...
            await AdminService.updateMaterial(id, { name });
            this.error = "Материал обновлен";
            this.getAll(); // Обновляем список после изменения
        } catch (error) {
            this.error = error.response?.data?.error || "Ошибка при обновлении материала";
        }
    },
    async delete_() {
        try {
