<template> 
  ...
  <div class="main">
    <div class="container">
      <h1>ДОБАВЛЕНИЕ</h1>
      <label for="description">Сезон:</label>
      <input
        type="text"
        name="description"
        v-model="description"
    ...
    <button class="show-button" @click="getAll">Показать все сезоны</button>
...
export default {
    ...
    async add() {
        try {
            await AdminService.addSeason({ name: this.description });
            this.error = "Сезон добавлен";
            this.getAll(); // Обновляем список после добавления
        } catch (error) {
            this.error = error.response?.data?.error || "Ошибка при добавлении сезона";
        }
    },
    async getAll() {
        try {
            const response = await AdminService.getSeason();
            this.products = response.data; // Исправлено, убран лишний `if`
        } catch (error) {
            this.error = error.response?.data?.error || "Ошибка при получении сезона";
        }
    },
    async update() {
        try {
            ...
            await AdminService.updateSeason(id, { name });
            this.error = "Сезон обновлен";
            this.getAll(); // Обновляем список после изменения
        } catch (error) {
            this.error = error.response?.data?.error || "Ошибка при обновлении сезона";
        }
    },
    async delete_() {
        try {
            ...
            this.error = "Сезон удален";
            this.getAll(); // Обновляем список после удаления
        } catch (error) {
            this.error = error.response?.data?.error || "Ошибка при удалении сезона";
        }
    }
    }
}
